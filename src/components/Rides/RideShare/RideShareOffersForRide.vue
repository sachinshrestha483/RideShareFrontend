<template>
  <div class="pageMargin1">
    <div class="flex flex-col">
      ...
      <div>
        {{ currentIndex }}
        Prev Possible{{ isPrevPossible }} isNextPossible{{
          isNextPossible
        }}
        list length {{ list.length }}

        <!-- current Ride Share Offer ->   {{currentRideShareOffer}}  {{list}} -->
      ...
      <div v-if="dataLoaded && currentRideShareOffer != null">
        <div :key="currentRideShareOffer">
          ...
          <UserInfo
            :userId="currentRideShareOffer.userId"
            :title="`User Info`"
          />
          <div class="my-4"></div>
          <NotesForRideCreator
            :bodyText="currentRideShareOffer.notesForRideCreater"
            :title="`Nodes For Ride Publisher`"
          />
...
import {
  getAllRideShareOfferForUserRide,
  getRideShareOfferById,
  getAllDisApprovedRideShareOfferids,
...
export default {
  ...
  setup(porps) {
    ...
    const getData = async () => {
      const response = await getAllDisApprovedRideShareOfferids(porps.rideId);
