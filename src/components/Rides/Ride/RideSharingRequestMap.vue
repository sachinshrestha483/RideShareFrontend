...
export default {
  ...
  setup(props) {
    ...
    const DrawOverlappingRoute = ( ) => {
      let r =  props.overlappingPath ;
      if (r.length > 0) {
    ...
    const drawMapMarkers = () => {
      ...
      var searchedInitialPositionmarkerOptions = {
        title: "Initial Position marker",
        clickable: true,
      };

      var searchedInitialPositionmarker = L.marker(
        [props.searchedStartLocation.lat, props.searchedStartLocation.lon],
        searchedInitialPositionmarkerOptions
      );
      searchedInitialPositionmarker.addTo(mymap);

      var searchedfinalmarkerOptions = {
        title: "Final Position Marker",
      };
      var searchedFinalPositionmarker = L.marker(
        [
          props.searchedEndLocation.lat,
          props.searchedEndLocation.lon,
        ],
        searchedfinalmarkerOptions
      );
